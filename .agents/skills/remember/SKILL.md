...
This skill must never persist secrets. If any sensitive value appears in the conversation or context, do not copy it to `.agents/memory.md`.
...
Before writing `.agents/memory.md`, run a final pass over the content to ensure no sensitive value is present.
...
If `.agents/memory.md` already exists, show the developer a brief summary of what is currently saved and ask for confirmation before overwriting:

Step 1 — Read `.agents/memory.md`, provide the one-line summary, and stop to wait for developer input:

```
.agents/memory.md already exists from a previous session.
...
- If they say **yes**, write the new `.agents/memory.md`.
- If they say **no**, do not write anything and reply:

```
No changes made. .agents/memory.md is unchanged.
```
...
```
Memory saved to .agents/memory.md.
...
```
No .agents/memory.md found in this project.
...
Read `.agents/memory.md` first. Then check for these specific context files if they exist and read only those:
...
If `.agents/memory.md` exists but is missing important context, say so honestly:

```
I found .agents/memory.md but some context seems missing —
